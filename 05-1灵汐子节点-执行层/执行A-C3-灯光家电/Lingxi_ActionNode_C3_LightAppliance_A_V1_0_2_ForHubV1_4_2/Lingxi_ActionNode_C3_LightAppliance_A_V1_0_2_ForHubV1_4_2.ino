...
#define LINGXI_NODE_FIRMWARE             "Lingxi_ActionNode_C3_LightAppliance_A_V1_0_2_ForHubV1_4_2"
#define LINGXI_NODE_VERSION              "执行层子节点A V1.0.2 C3通用板 / 兼容中控 V1.4.2"
...
//
// A 模式实际接口：
//   客厅灯：GND IO10 VCC GND
//   卧室灯：GND IO4  VCC GND
//   四路继电器：VCC IO2 IO3 IO0 IO1 GND
//
// C3 引脚约定：
...
#define LIVING_LIGHT_PIN                 10
#define BEDROOM_LIGHT_PIN                4
#define LIVING_LIGHT_NUM                 8
#define BEDROOM_LIGHT_NUM                8

#define RELAY_AIR_PIN                    2
#define RELAY_TV_PIN                     3
#define RELAY_FRIDGE_PIN                 0
#define RELAY_WATER_HEATER_PIN           1
...
void setup() {
  ...
  Serial.println();
  Serial.println("========================================");
  Serial.println("灵汐执行层子节点A - 灯光家电（C3通用板）");
  ...
  // 重要修复：ESP32-C3 上 WebServer.begin() 必须放在 WiFi.mode()/softAP()/begin() 之后。
