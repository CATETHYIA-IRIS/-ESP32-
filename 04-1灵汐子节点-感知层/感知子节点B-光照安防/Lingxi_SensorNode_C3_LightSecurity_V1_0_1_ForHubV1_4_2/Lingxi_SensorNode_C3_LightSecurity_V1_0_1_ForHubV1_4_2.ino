...
#define LINGXI_PUBLIC_VERSION           "感知层子节点B V1.0.3 / 兼容中控 V1.4.2"
#define LINGXI_NODE_ID                  "SensorNode-LightSecurity-01"
#define LINGXI_NODE_NAME                "感知层子节点B-光照安防"
#define LINGXI_NODE_TYPE                "sensor_board"
#define LINGXI_NODE_ROLE                "sensor"
#define LINGXI_NODE_FIRMWARE            "Lingxi_SensorNode_C3_LightSecurity_V1_0_3_StatusLightPatch_ForHubV1_4_2"
...
// 注意：用户指定全称“灵汐-感知层子节点B光照安防”超过 32 字节 SSID 上限。
// 实际热点名使用短中文名，保证 ESP32 SoftAP 可稳定启动。
#define LINGXI_REQUESTED_AP_SSID        "灵汐-感知层子节点B光照安防"
#define LINGXI_CONFIG_AP_SSID           "灵汐-感知B光照安防"
...
#define SMOKE_ADC_PIN                   0      // MQ2 烟雾，沿用原集成板 GPIO0
#define BEDROOM_PIR_PIN                 10     // 卧室人体，沿用原集成板 GPIO10
#define MAIN_DOOR_CONTACT_PIN           2      // 大门门磁，沿用原集成板 GPIO2
#define RGB_PIN                         8
#define RGB_NUM                         1
...
// 函数说明：读取 ADC 多次取平均，用于 MQ2 烟雾模拟量。
...
// 函数说明：向 BH1750 写入控制命令。
// 可改范围：禁止修改 I2C 地址，除非硬件地址跳线改变。
...
// 函数说明：初始化 BH1750 光照传感器。
// 可改范围：只允许根据硬件更改 SDA/SCL 引脚。
...
// 函数说明：把 lux 映射为 0~100 百分比，用于兼容中控页面。
// 可改范围：500 lux 映射上限可按现场光照调整。
...
// 函数说明：读取 MQ2 烟雾模拟量并计算报警标志。
// 可改范围：阈值 SMOKE_ALARM_THRESHOLD 可按实测校准。
void readSmokeMq2() {
...
String endpointsJson() {
  String json = "[";
  json += "{\"id\":\"human_bedroom\",\"name\":\"卧室人体\",\"type\":\"sensor\",\"capability\":\"presence\",\"room\":\"光照安防\"},";
  json += "{\"id\":\"smoke\",\"name\":\"烟雾传感器\",\"type\":\"sensor\",\"capability\":\"smoke\",\"room\":\"光照安防\"},";
  json += "{\"id\":\"door_main_contact\",\"name\":\"大门门磁\",\"type\":\"sensor\",\"capability\":\"contact\",\"room\":\"光照安防\"},";
  json += "{\"id\":\"light_level\",\"name\":\"环境光照\",\"type\":\"sensor\",\"capability\":\"light\",\"room\":\"光照安防\",\"unit\":\"%\"}";
  json += "]";
  return json;
}
...
void setup() {
  ...
  Serial.println();
  Serial.println("========================================");
  Serial.println("灵汐全屋智能控制中心 - 感知层子节点B光照安防");
